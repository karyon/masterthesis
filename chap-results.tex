...
\begin{table}[h]
    \begin{center}
        \begin{tabulary}{0.98\textwidth}{| L | L | L | L | L |}
            ...
            50.4\,ms & 3.38\,ms & 14.91 & 0.63\,ms & 12.57\\
            \hline
        \end{tabulary}
        \caption{Timings of interleaved shading (IS) with a block size of 4x4. Note that the blur pass is mandatory when using interleaved shading, the middle columns merely illustrates the efficiency of the presented implementation, while the last column shows the practical speedup that is achieved.}
        \label{tab:results:timings_interleaved_shading}
    \end{center}
\end{table}


\Cref{tab:results:timings_interleaved_shading} shows timings of the presented implementation. Surprisingly, the measurements show a speedup of 16.4 when not considering the additional blur phase. Most notably, the interleaved shading technique introduces overhead, as indicated by the speedup of 14.91, which is considerably lower than the theoretical maximum of 16. In absolute terms, a speedup of 16 would require the final gathering phase to take 3.15\,ms. As a result, the technique introduces 0.23\,ms of overhead.

When considering the blur phase, the overhead adds up to 0.86\,ms and the speedup is lowered to 12.57. Of course this is still a substantial improvement, especially considering the negligible quality impact.
